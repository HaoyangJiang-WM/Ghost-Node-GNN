...
def val_step(model, val_loader, criterion, device, load_path="node_classes.npy"):
    ...
    # 遍历验证集的批量图数据
    with torch.no_grad():  # 关闭梯度计算，加快验证过程
        with tqdm(val_loader, desc="Validating") as pbar:
            for batch in pbar:
                ...
                # === 计算偏移量 ===
                # 获取批量中每个图的起始偏移量
                node_offset = torch.cumsum(torch.bincount((batch.batch)), dim=0)  # 每个图的起始位置
                # node_offset = torch.cumsum(torch.bincount((batch.batch[batch.mask])), dim=0)  # 每个图的起始位置
                node_offset = torch.cat([torch.tensor([0], device=device), node_offset[:-1]])  # 偏移量
...
def train(model, dataset, hparams):
    ...
    # criterion = mse_loss  # 不再使用 interestingness_score
