...
def construct_model(hparams, dataset):
    # --- 修复: 使用 edge_attr_with_ghosts 来初始化模型 ---
    ...
    # 为了完整性，保留其他模型的构造，尽管它们未在models_o.py中完全定义
    # 如果要使用它们，需要将它们也更新为继承自 BaseModel
    # ...
...
# ... (后续所有函数 train, save_checkpoint, evaluate_nse 等保持不变) ...
# 注意：如果 evaluate_nse 也需要被使用，其中的 model(...) 调用也需要像 train_step 一样被更新
# ---
