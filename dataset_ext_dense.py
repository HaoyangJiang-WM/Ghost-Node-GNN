# 文件名: lamah_dataset_definitive_final.py
# 描述: 最终完整版。以高级鬼点代码为100%基础，精确集成了下游密集连接功能，保留所有原有方法和细节。
...
class LamaHDataset(Dataset):
    """
    一个用于LamaH-CE水文数据集的PyTorch Geometric数据集类。
    此版本以高级鬼点实现为基础，并集成了下游密集连接功能。
    """
    ...
    def __init__(self, root_dir, years=range(2000, 2018), root_gauge_id=399, rewire_graph=True,
        ...
        # 使用 list.append 和 map(torch.stack, ...) 的原始数据加载方式 (来自“代码1”)
    ...
    def build_dense_connections(self):
        """
        构建密集连接矩阵，其中每个节点连接到其所有下游节点。
        此方法基于 self.original_edge_index 操作。
        """
