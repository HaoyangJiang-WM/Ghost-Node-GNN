...
class LamaHDataset(Dataset):
    ...
    def __init__(self, root_dir, years=range(2000, 2018), root_gauge_id=399, rewire_graph=True,
        ...
        self.original_edge_index = self.edge_index.clone()
        self.original_edge_attr = self.edge_attr.clone()
    ...
    def _has_valid_data(self, gauge_id):
        ...
        if (q_df[self.Q_COL] > 0).all() and (q_df[self.Q_COL] <= 1e30).all():
            ...
            if len(q_df) == (18 * 365 + 5) * 24 and len(met_df) == (18 * 365 + 5) * 24:
    ...
    def create_ghost_graph_structure(self):
        ...
        for i, init_node in enumerate(initial_nodes):
            ...
            try:
                ...
                new_attrs.append([0.0] * edge_attr.shape[1])
        if new_edges:
            ...
            edge_index_with_ghosts = edge_index
            edge_attr_with_ghosts = edge_attr
    ...
    def get(self, idx):
        ...
        if self.ghost_node_map:
            for ghost_idx in sorted(self.ghost_node_map.keys()):
                ...
                placeholder_ghost_features.append(x_original[downstream_node_idx])
        ...
        if self.ghost_node_map:
            for ghost_idx in sorted(self.ghost_node_map.keys()):
                ...
                x_initial = x_original[initial_node_idx]
                x_downstream = x_original[downstream_node_idx]

                if self.use_feature_extrapolation:
                    # extropolate for initial
                    last_step_i = x_initial[-1, :]
                    second_last_step_i = x_initial[-2, :]
                    step_diff_i = last_step_i - second_last_step_i
                    future_steps_i = torch.stack([last_step_i + (i + 1) * step_diff_i for i in range(self.lead_time)])
                    x_initial = torch.cat([x_initial[self.lead_time:], future_steps_i], dim=0)
                    # extropolate for downstream
                    last_step_d = x_downstream[-1, :]
                    second_last_step_d = x_downstream[-2, :]
                    step_diff_d = last_step_d - second_last_step_d
                    future_steps_d = torch.stack([last_step_d + (i + 1) * step_diff_d for i in range(self.lead_time)])
        ...
        return Data(x=x_final, y=y.unsqueeze(-1),
                    edge_index=self.edge_index_with_ghosts,
                    edge_attr=self.edge_attr_with_ghosts,
                    mask=real_node_mask,
                    ghost_mask=ghost_node_mask,
                    x_ghost_concat=x_ghost_concat)
